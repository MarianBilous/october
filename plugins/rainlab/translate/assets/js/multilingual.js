...
+function ($) { "use strict";
    ...
    var MultiLingual = function(element, options) {
        ...
        this.$dropdown.on('click', '[data-switch-locale]', this.$activeButton, function(event){
            var selectedLocale = $(event.currentTarget).data('switch-locale')
            $('[data-switch-locale="' + selectedLocale + '"]').submit()
        })

        this.$dropdown.on('submit', '[data-switch-locale]', this.$activeButton, function (e) {
            var currentLocale = e.data.text()
            var selectedLocale = $(e.currentTarget).data('switch-locale')
